 ...
 * `futex_wait(word, expected)`/`futex_wake_one(word)`/
 * `futex_wake_all(word)` -- the building block a handful of higher-level
 ...
 * `futex_word`/`futex_wait`/`futex_wake_one`/`futex_wake_all`, e.g. for a
 * FreeBSD **kernel** `msleep(9)`/`wakeup(9)` backend), or none defined
 * (default: a portable "parking lot" fallback in `futex_std.ipp`, built
 * entirely on `mutex.hpp`'s `mutex`/`condition_variable`, always
 * available on any hosted target). See [`docs/futex.md`](../../docs/
 * futex.md) for the full backend writeup, the `RELOCO_FUTEX_BACKEND_
 * CUSTOM` FreeBSD-kernel example, and the parking-lot collision-safety
 * argument.
 *
 * Every `futex_wait`/`futex_wake_one`/`futex_wake_all` declaration below
 * is `RELOCO_API`-decorated (see `detail/compat.hpp`) so the built-in
 * fallback backend participates in reloco's header-only/`RELOCO_SHARED`
 * split exactly like `mutex.hpp`'s own classes: out-of-line definitions
 * live in `futex_std.ipp`/`futex_linux.ipp`/`futex_freebsd.ipp`, included
 * from here only when `RELOCO_SHARED_PROVIDE_DEFINITIONS` is `1`.
 *
