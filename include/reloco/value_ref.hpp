...
public:
  ...
  constexpr explicit value_ref(U &val RELOCO_LIFETIMEBOUND RELOCO_LIFETIME_CAPTURE_BY_THIS) noexcept
      : m_ptr(std::addressof(val)) {}

  /**
   * @brief Rejects rvalue and temporary object bindings.
   */
  template <typename U, std::enable_if_t<!std::is_lvalue_reference_v<U>, int> = 0> constexpr value_ref(U &&) = delete;

  [[nodiscard]] constexpr T *get() const noexcept RELOCO_LIFETIMEBOUND { return m_ptr.get(); }

  [[nodiscard]] constexpr T &operator*() const noexcept RELOCO_LIFETIMEBOUND {
  ...
  [[nodiscard]] constexpr T *operator->() const noexcept RELOCO_LIFETIMEBOUND { return m_ptr.get(); }
   ...
   */
  [[nodiscard]] constexpr value_ptr<T> pointer() const noexcept { return m_ptr; }
