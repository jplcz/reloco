...
TEST(SsoFlatMapTest, DefaultConstruction) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
...
TEST(SsoFlatMapTest, AllocateWithCapacity) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create(16);
  ASSERT_TRUE(map_res.has_value());
...
TEST(SsoFlatMapTest, MaintainsSortedOrderByKey) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
  auto &map = *map_res;
...
TEST(SsoFlatMapTest, InsertionPastInlineCapacityPromotesToHeap) {
  auto map_res = reloco::sso_flat_map<int, int, 2>::try_create();
  ASSERT_TRUE(map_res.has_value());
  auto &map = *map_res;
  ...
  // Unlike inline_flat_map, insertion past InlineCapacity never fails with
  // error::capacity_exceeded: sso_flat_map just grows onto the heap.
...
TEST(SsoFlatMapTest, EnforcesUniqueKeys) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
  auto &map = *map_res;
...
TEST(SsoFlatMapTest, ContainsAndTryAt) {
  auto map_res = reloco::sso_flat_map<int, std::string, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
  auto &map = *map_res;
...
TEST(SsoFlatMapTest, TryAtAllowsMutatingMappedValue) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
  auto &map = *map_res;
...
TEST(SsoFlatMapTest, TryAtConstReturnsReadOnlyReference) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
...
TEST(SsoFlatMapTest, TryRemoveErasesEntry) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
  auto &map = *map_res;
...
TEST(SsoFlatMapTest, ClearResetsSize) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
  auto &map = *map_res;
...
TEST(SsoFlatMapTest, DeepCloneWithExplicitAllocator) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
...
TEST(SsoFlatMapTest, MutableContainerRefIntegration) {
  auto map_res = reloco::sso_flat_map<int, int, 4>::try_create();
  ASSERT_TRUE(map_res.has_value());
  auto &map = *map_res;
...
TEST(SsoFlatMapTest, IsNeverTriviallyRelocatableRegardlessOfMappedType) {
  EXPECT_FALSE((reloco::is_trivially_relocatable_v<reloco::sso_flat_map<int, int, 4>>));
  EXPECT_FALSE((reloco::is_trivially_relocatable_v<reloco::sso_flat_map<int, std::string, 4>>));
}
